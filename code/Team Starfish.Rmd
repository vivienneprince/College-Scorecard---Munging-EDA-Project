...
```{r}
# Import raw data
raw.df <- read.csv("https://github.com/vivienneprince/College-Scorecard---Munging-EDA-Project/raw/main/data/Starfish_Fields%20-%20Sheet1.csv")
...
df2014
Observed Colleges in the Southeast region
#2015
df2015 <- sc_init() %>% 
  sc_filter(region == 5,preddeg == 3) %>% 
  sc_select( instnm, stabbr, sat_avg, tuitionfee_in, pctpell
...
df18_pl
```
Viewing the relationship between the Average SAT Score in the SE region and the percentage of Pell Grant Students. It appears that the lower the SAT score the hight the University's Pell Grant Percentage. What's going on?
...
```
Once data is subset by Tuition it looks like many of the Above Average and Expensive Univserity do not have many students that recieved pell grants and also have higher Avergae SAT Scores
...
```{r}
#Pull the age minimum age entry and share of students that have recievied a loan
...
```
View cluster of people that have recieved loan and at what age at the the institution. 
